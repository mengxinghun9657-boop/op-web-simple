...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='AI查询审计日志表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='报告索引表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='用户备忘表';
...
-- AI 智能查询功能初始化完成
SELECT 'AI 智能查询功能数据库表初始化完成！' AS ai_query_message;
...
-- 注意: 如果是从旧版本升级，routing_rules 表可能有 rule_metadata 列
-- 需要手动执行修复脚本: fix_routing_tables.sql
-- 新部署会直接创建正确的 metadata 列，无需修复
...
CREATE TABLE IF NOT EXISTS `routing_logs` (
    ...
    `intent_type` VARCHAR(50) NOT NULL COMMENT '路由到的意图类型',
    `confidence` DECIMAL(5, 4) NOT NULL COMMENT '置信度',
    `routing_method` VARCHAR(50) NOT NULL COMMENT '路由方法',
...
CREATE TABLE IF NOT EXISTS `routing_feedback` (
    ...
    `routing_log_id` INT NOT NULL COMMENT '路由日志ID',
    `correct_intent` VARCHAR(50) NOT NULL COMMENT '正确的意图类型',
    `user_id` VARCHAR(100) NOT NULL COMMENT '反馈用户ID',
    `comment` TEXT COMMENT '反馈备注',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    
    INDEX `idx_routing_feedback_routing_log_id` (`routing_log_id`),
    INDEX `idx_routing_feedback_user_id` (`user_id`),
    INDEX `idx_routing_feedback_created_at` (`created_at`),
    FOREIGN KEY (`routing_log_id`) REFERENCES `routing_logs`(`id`) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS `rule_suggestions` (
    ...
    `pattern` VARCHAR(500) NOT NULL COMMENT '建议的查询模式',
    `suggested_intent` VARCHAR(50) NOT NULL COMMENT '建议的意图类型',
    `confidence` DECIMAL(5, 4) NOT NULL COMMENT '建议置信度',
    `support_count` INT NOT NULL DEFAULT 0 COMMENT '支持证据数量',
    `evidence` JSON COMMENT '支持证据(反馈记录)',
    `status` VARCHAR(20) NOT NULL DEFAULT 'pending' COMMENT '状态(pending/approved/rejected)',
    `adopted_by` VARCHAR(100) COMMENT '采纳者',
    `adopted_at` DATETIME COMMENT '采纳时间',
    `created_rule_id` INT COMMENT '创建的规则ID',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    
    INDEX `idx_rule_suggestion_status` (`status`),
    INDEX `idx_rule_suggestion_suggested_intent` (`suggested_intent`),
    INDEX `idx_rule_suggestion_created_at` (`created_at`),
    FOREIGN KEY (`created_rule_id`) REFERENCES `routing_rules`(`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='规则建议表';

SELECT '✅ 路由规则相关表初始化完成！' AS routing_message;
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='路由规则模板表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='路由规则草稿表';

SELECT '✅ 路由规则智能辅助数据库表初始化完成！' AS routing_assistant_message;
