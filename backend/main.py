...
async def lifespan(app: FastAPI):
    ...
    if ENABLE_SCHEDULER:
        ...
        logger.info("ℹ️ 已禁用定时调度器初始化（ENABLE_SCHEDULER=false）")
    
    # 启动文件监控服务
    # 仅当监控源目录存在时才初始化（外网环境未挂载该目录，跳过以避免无意义的等待）
    file_watcher = None
    alerts_source_path = "/app/alerts_source"
    if ENABLE_FILE_WATCHER and os.path.exists(alerts_source_path):
        try:
        ...
        if not ENABLE_FILE_WATCHER:
            logger.info("ℹ️ 已禁用文件监控服务初始化（ENABLE_FILE_WATCHER=false）")
        else:
    ...
    if ENABLE_SCHEDULER:
        try:
            ...
            logger.warning(f"⚠️ 停止定时调度器失败: {e}")
...
# APIServer 监控告警路由
from app.api.v1 import apiserver_alerts
app.include_router(apiserver_alerts.router, prefix=settings.API_V1_PREFIX, tags=["APIServer 监控告警"])
...
# CCE 集群监控路由
from app.api.v1 import cce_monitoring
app.include_router(cce_monitoring.router, prefix=settings.API_V1_PREFIX, tags=["CCE集群监控"])
