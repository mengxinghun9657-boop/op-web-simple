...
class BOSAnalyzerAI:
    ...
    def detect_anomalies(self, results: List[Dict]) -> List[Dict]:
        """
        检测异常Bucket（基于3-sigma统计方法）
        
        原理：
        1. 计算所有Bucket存储空间的平均值（μ）和标准差（σ）
        2. 设定异常阈值为 μ + 3σ（正态分布中99.7%的数据会落在±3σ范围内）
        3. 超过阈值的Bucket被标记为异常
        4. 计算偏离程度 = (存储空间 - μ) / σ
        
        作用：
        - 识别存储空间显著偏离正常范围的Bucket
        - 发现可能的数据冗余或不合理使用
        - 辅助容量规划和成本优化
        - 无需历史数据，基于当前数据的横向对比
    ...
    def generate_recommendations(self, results: List[Dict], anomalies: List[Dict], low_usage: List[Dict]) -> List[Dict]:
        ...
        if anomalies:
            recommendations.append({
                ...
                'priority': 'high',
                'title': '存储空间异常警告',
                'description': f'检测到 {len(anomalies)} 个Bucket存储空间显著偏离正常范围',
                'affected_buckets': [a['bucket'] for a in anomalies[:5]],
                'action': '建议检查这些Bucket的存储空间使用情况，确认是否为正常业务需求或存在数据冗余'
            })
...
包含 <strong>{summary['total_objects']:,}</strong> 个文件。</p>

<h3>关键洞察</h3>
<ul>
    <li><strong>存储集中度</strong>: TOP 10 Bucket占总存储的 {top10_percentage:.1f}%，存储资源高度集中</li>
    <li><strong>异常检测</strong>: 发现 {len(anomalies)} 个存储空间显著偏离正常范围的Bucket，需要重点关注</li>
...
"""
    ...
    def _format_top_buckets(self, buckets: List[Dict]) -> str:
        ...
        for i, bucket in enumerate(buckets, 1):
            ...
            lines.append(f"{i}. {bucket['bucket']}: {space_gb:.2f} GB, {bucket['objects']:,}个文件")
        
        return "\n".join(lines)
    async def generate_html_report(self, results: List[Dict]) -> str:
        """生成的倒金字塔HTML报告"""
        summary = self.get_summary_stats(results)
        ...
        ai_summary = await self.generate_ai_summary(results, anomalies, recommendations)  # 添加 await
    ...
    <div class="container">
        ...
        <div class="section">
            ...
            <div style="background: #F3E5F5; padding: 20px; border-radius: 10px; margin-bottom: 20px; border-left: 4px solid #9C27B0;">
                <h3 style="color: #9C27B0; margin-top: 0;">🔬 3-Sigma统计检测原理</h3>
                <p style="margin: 10px 0;"><strong>什么是3-Sigma方法？</strong></p>
                <p style="margin: 10px 0;">3-Sigma（三倍标准差）是一种基于正态分布的统计异常检测方法。在正态分布中，约99.7%的数据会落在平均值±3倍标准差的范围内，超出此范围的数据被视为统计异常。</p>
                
                <p style="margin: 15px 0 10px 0;"><strong>检测步骤：</strong></p>
                <ol style="margin: 5px 0 5px 20px; line-height: 1.8;">
                    <li>计算所有Bucket存储空间的<strong>平均值（μ）</strong></li>
                    <li>计算<strong>标准差（σ）</strong>，衡量数据的离散程度</li>
                    <li>设定异常阈值：<strong>μ + 3σ</strong></li>
                    <li>存储空间超过阈值的Bucket被标记为异常</li>
                    <li>计算偏离程度：<strong>(存储空间 - μ) / σ</strong></li>
                </ol>
                
                <p style="margin: 15px 0 10px 0;"><strong>作用与价值：</strong></p>
                <ul style="margin: 5px 0 5px 20px; line-height: 1.8;">
                    <li><strong>识别异常Bucket</strong>：快速发现存储空间显著偏离正常范围的Bucket</li>
                    <li><strong>成本优化</strong>：异常大的Bucket可能存在数据冗余或不合理使用</li>
                    <li><strong>容量规划</strong>：了解存储空间分布，预测未来容量需求</li>
                    <li><strong>风险预警</strong>：及时发现可能的存储空间异常或误操作</li>
                    <li><strong>无需历史数据</strong>：基于当前数据的横向对比，不依赖历史趋势</li>
                </ul>
                
                <p style="margin: 15px 0 0 0; color: #666; font-size: 0.9em;">
                    <strong>注意</strong>：偏离程度越大（如4σ、5σ），表示该Bucket的存储空间越异常，需要重点关注。
                </p>
            </div>
            
            <!-- 异常检测结果 -->
            <h3 style="margin-top: 30px;">检测结果</h3>
            {f'<table><thead><tr><th>Bucket名称</th><th>存储空间(PB)</th><th>文件数</th><th>偏离程度</th></tr></thead><tbody>{anomaly_rows}</tbody></table>' if anomalies else '<p style="color: #4CAF50; text-align: center; padding: 20px;">✓ 未检测到异常Bucket（所有Bucket存储空间均在正常范围内）</p>'}
        </div>
        ...
        <div class="section">
            ...
            <div id="top30Table" class="collapsible">
                <table>
                    <thead>
                        <tr><th>排名</th><th>Bucket名称</th><th>存储空间(PB)</th><th>文件数</th></tr>
                    </thead>
        ...
        {f'''<div class="section">
            ...
            <div id="lowUsageTable" class="collapsible">
                <table>
                    <thead>
                        <tr><th>Bucket名称</th><th>存储空间(PB)</th><th>文件数</th></tr>
                    </thead>
